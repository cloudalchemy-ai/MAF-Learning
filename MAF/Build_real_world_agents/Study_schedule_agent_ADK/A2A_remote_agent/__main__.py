...
from agent import study_schedule_agent # Renamed from create_agent
...
from agent_executor import StudyScheduleAgentExecutor
load_dotenv()
...
def main():
    ...
    try:
        ...
        # Define skills relevant to study scheduling
        skill_study_schedule = AgentSkill(
            id="study_schedule", # Matches the tool name in conceptualizer_agent.py
            name="Generate Study Schedule",
            description="Generates a study schedule for a student learning a specific topic.",
            tags=["education", "study_schedule", "time_management"],
            examples=["Create a study schedule for a student learning Python programming.", "Plan a study timetable for a high school math student."],
        )
        # Add more skills if your conceptualizer has other distinct tools
        # skill_market_research = AgentSkill(...)

        agent_card = AgentCard(
            name="study_schedule", # IMPORTANT: This MUST match the name HostAgent tries to connect to
            description="An agent that specializes in generating study schedules for students.",
            ...
            skills=[skill_study_schedule], # List all relevant skills here
        )

        # --- Initialize the Google ADK Agent ---
        # This imports the agent definition (with its instruction and tools)
        adk_agent = study_schedule_agent() # Calls a function from conceptualizer_agent.py
        ...
        agent_executor = StudyScheduleAgentExecutor(runner)
